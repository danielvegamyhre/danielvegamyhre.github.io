...
As you can see, we have a 3 stage pipeline where:
- The producer warp is responsible for issuing async TMA loads of A/B/SFA/SFB tiles from GMEM to SMEM. We have a queue (circular buffer) of these SMEM tiles. Once we wraparound, before re-using any buffer, the producer waits on the completion of the `smem_empty_mbar` mbarrier for that buffer. We will start with tile sizes:
  - `BM=128`, `BN=128`, `BK=128`
- The consumer warp runs **only on CTA 0** in the pair, since only one CTA issues the 2CTA `tcgen05.mma` instruction. It is responsible for iterating through the A/B tiles, in increments dictated by the `tcgen05.mma` dimensions selected. We'll start with: `MMA_M=128`, `MMA_N=128`, `MMA_K=32`
  ...
  - For `K/BK` iterations (we tile over the K dimension in increments of `BK`), the consumer does the following:
    - For each `BK=128` tile, we'll have 4 `MMA_K=32` iterations, issuing `tcgen05.mma` instructions, accumulating the outputs in a given TMEM buffer.
        - Before each `tcgen05.mma`, we need to load the SFA/SFB tiles from SMEM to TMEM - as we established above, this is required for the instruction. 
