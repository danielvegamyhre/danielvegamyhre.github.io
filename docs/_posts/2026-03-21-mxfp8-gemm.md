...
7. **Optimization 8**: New epilogue strategy: TMEM -> REG -> pipelined TMA stores
...
Before we start on an initial kernel design, it is important to understand the requirements of the core PTX instruction we'll used for the block scaled matrix-multiply accumulate operation: `tcgen05.mma.cta_group::2.kind::mxf8f6f4.block_scale.block32`.
...
- `a-desc` or `[a-tmem]`= For the A tile, we can either store it in SMEM and pass a [SMEM descriptor](https://docs.nvidia.com/cuda/parallel-thread-execution/#tcgen05-shared-memory-descriptor), or store it in TMEM and pass the TMEM base address. The bracket notation is PTX syntax for deferencing.
