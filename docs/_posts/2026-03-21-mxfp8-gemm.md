...
In blocked layout, each 16 element wide "superrow" *within* a scale factor tile is contiguous in memory, and each full scale factor tile itself occupies one contiguous 512 byte chunk of memory.
...
Understanding 2 CTA `tcgen05.mma` synchronization patterns is crucial for this kernel design. At a high level, the dependency chart looks like this:
