...
To summarize the 2 CTA `tcgen05.mma` flow before diving deeper:
1. **Both CTAs**: Producer loads A/B tiles from GMEM -> SMEM:
    ...
    - Issue TMA loads for SFA tile shape (BM, BK//32) and SFB tile shape (BK//32, BM). Note SFA is completely different for both CTA 0 and CTA 1, but SFB actually has to be *replicated* on both CTAs! This is not officially documented by NVIDIA anywhere that I'm aware of, but luckily I had seen this [tweet](https://x.com/gaunernst/status/2008183108999487627?s=20) from Thien about NVFP4 GEMMs prior to starting this work, and I assumed this same oddity might apply to MXFP8 as well, and so it does!
2. **CTA 0 only**: Consumer copies SFA/SFB from SMEM into TMEM
