...
#### Core Matrices
The key insight to understanding why this concept exists is that *tensorcores are not actually aware of individual elements*, they do not stride through data at row/column level granularity. Tensorcores only understand *matrices* - specifically, `8x16B` "core matrices." They view the A/B tiles as a grid of these core matrices. This suggest some kind of systolic array or similar hardware unit physically implemented to operate on this tile size.
...
The producer will use these tensormaps to issue TMA loads like so:
...
```c++
    ...
    for (int group_id = start_group_id; group_id < total_groups; group_id += num_groups) {
        ...
        for (int block_k_idx = 0; block_k_idx < num_blocks_k; block_k_idx++) {
            ...
            // arrive and tell CTA 0 mbar how many bytes to expect!
            ...
            const uint32_t k_off_128 = (uint32_t)(global_k_off / 128);
            const uint32_t k_off_sf = (uint32_t)(global_k_off / 128);  
