...
2. Last but not least: **deadlocks**. There are so many things that could cause this in warp specialized kernels on Blackwell with producer warp, consumer warp, epilogue warp group, double buffered TMEM, persistent scheduling, and complex synchronization to orchestrate it all safely... how can we narrow down the issue *efficiently*?
