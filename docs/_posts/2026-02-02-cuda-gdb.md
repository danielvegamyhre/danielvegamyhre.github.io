...
2. Last but not least: **deadlocks**. There are so many things that could cause this. Taking a warp-specialized persistent GEMM kernel for sm100 as an example:
    - Producer (TMA load warp) <-> consumer (MMA warp) coordination for safely re-using SMEM buffers in the queue
    - Cross CTA mbarrier signaling via DSMEM or `tcgen05.commit` multicast for 2 CTA `tcgen05.mma` coordination.
    - Consumer -> epilogue warpgroup signaling when TMEM buffer with accumulated results is ready for processing and storage to GMEM (and which TMEM buffer, if double-buffering!)
    - Epilogue warpgroup -> consumer signaling when TMEM buffer can be safely re-used
    - In all of the above, is there a logical error, a race condition, a simple offset calculation typo, or a fundamental misunderstanding of the synchronization semantics of these PTX instructions? Who knows!

Sometimes you can resolve these through code analysis, print statements, or trial and error. However, as the synchronization patterns / dependency graphs grow in complexity, the need arises for a method that is at least slightly more systematic and efficient.
...
### Investigating the Consumer Warp by Switching Thread Context
