...
<img src="/images/cuda-gdb/2cta.png" alt="2cta-mma" style="width: 100%; max-width: 1200px; height: auto;">

Again, for more details on `tcgen05` PTX instruction semantics and Blackwell GEMM kernel design, I refer you to gaunerst's [blog](https://gau-nernst.github.io/tcgen05).
...
I can now see that for warp 1 (epilogue warp), it's hung waiting for the MMA mbarrier that signals the target TMEM buffer is ready for use by the epilogue (i.e., all relevant `tcgen05.mma` instrucions have completed).
...
> **Optional context**
> It is important because it does the following:
...
> - After we complete ALL BK tiles, we `tcgen05.commit` multicast again to signal epilogue warpgroups on both CTAs that the target TMEM buffer is ready for use.
