...
1. Run your code with `cuda-gdb <executable>` or `cuda-gdb --args <program>` (e.g. `cuda-gdb --args python -m pytest test.py`)
...
- `info cuda clusters` - info on threadblock clusters (on Hopper and later)
- `info cuda barriers` - info on barrier objects
